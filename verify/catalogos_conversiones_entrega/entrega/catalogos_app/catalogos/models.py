...
class ProductoMetricaConversion(models.Model):
    ...
    def equivalencia_texto(self):
        cantidad_origen = self.cantidad_origen.normalize() if self.cantidad_origen is not None else Decimal("0")
        factor = self.factor_conversion.normalize() if self.factor_conversion is not None else Decimal("0")
